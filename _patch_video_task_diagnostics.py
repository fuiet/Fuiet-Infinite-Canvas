...
new="""function nodeTaskVisualState(n){const wf=workflowNodeStatus(n.id).status;if(wf)return wf;if(n.frozen)return'frozen';const s=String(n.taskStatus||'');if(['queued','polling','retrying','running','fallback','provider_succeeded','result_pending'].includes(s))return s==='queued'?'pending':'running';if(['succeeded','failed','canceled'].includes(s))return s;return''}
  function safeTaskDiagnosticText(value){let text=String(value??'');text=text.replace(/Bearer\\s+[A-Za-z0-9._~+\\/=-]+/gi,'Bearer [redacted]');text=text.replace(/((?:x-api-key|api-key|authorization)\\s*[:=]\\s*)[^\\s,;]+/gi,'$1[redacted]');return text.slice(0,500)}
...
app=replace_once(app,"""async function monitorNodeTask(n,taskId,attempt,seedTask=null){
    let info=seedTask;
    while(true){""","""async function monitorNodeTask(n,taskId,attempt,seedTask=null){
    let info=seedTask;if(info)syncNodeTaskDiagnostics(n,info);
    while(true){""",'monitor seed diagnostics')
app=replace_once(app,"""info=(await apiJson('/api/tasks/'+encodeURIComponent(taskId))).task;
    const resultSyncing=""","""info=(await apiJson('/api/tasks/'+encodeURIComponent(taskId))).task;
    syncNodeTaskDiagnostics(n,info);
    const resultSyncing=""",'monitor poll diagnostics')

old_create="""n.taskStatus=created.task.status;n.taskProgress=created.task.progress||0;saveState();render();if(created.task.status==='succeeded')info=created.task;"""
new_create="""n.taskStatus=created.task.status;n.taskProgress=created.task.progress||0;syncNodeTaskDiagnostics(n,created.task);saveState();render();if(created.task.status==='succeeded')info=created.task;"""
app=replace_once(app,old_create,new_create,'created task diagnostics')

old_video="""body=`<div class=\"video-node-shell ${emptyVideo?'is-empty':'has-output'}\">${uploadAction}${media}${quick}</div>`;"""
new_video="""body=`<div class=\"video-node-shell ${emptyVideo?'is-empty':'has-output'}\">${uploadAction}${media}<div class=\"video-task-diagnostics-slot\" data-video-task-diagnostics>${videoTaskDiagnosticsHtml(n)}</div>${quick}</div>`;"""
app=replace_once(app,old_video,new_video,'video diagnostics slot')

old_visual="""const pct=status==='pending'?4:Math.max(8,Math.min(100,Number(n?.taskProgress||0)));$('i',bar).style.width=pct+'%'}else bar?.remove()});"""
new_visual="""const pct=status==='pending'?4:Math.max(8,Math.min(100,Number(n?.taskProgress||0)));$('i',bar).style.width=pct+'%'}else bar?.remove();const diag=$('[data-video-task-diagnostics]',el);if(diag&&n?.type==='video')diag.innerHTML=videoTaskDiagnosticsHtml(n)});"""
app=replace_once(app,old_visual,new_visual,'live diagnostics refresh')

app=replace_once(app,"""<small>${escapeHtml((t.error||t.id||'').slice(0,90))}</small>""","""<small>${escapeHtml((taskDiagnosticSummary(t)||t.error||t.id||'').slice(0,220))}</small>""",'task manager diagnostics')
...
display_test.write_text("""import test from 'node:test';
...
diag_test.write_text("""import test from 'node:test';
import assert from 'node:assert/strict';
import fs from 'node:fs';
