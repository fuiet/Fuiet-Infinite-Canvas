...
self.addEventListener('fetch',event=>{const url=new URL(event.request.url);if(url.origin!==self.location.origin||!url.pathname.startsWith(mediaPrefix))return;event.respondWith((async()=>{try{const id=decodeURIComponent(url.pathname.slice(mediaPrefix.length)),row=await getMedia(id);if(!row?.blob)return new Response('Media not found',{status:404,headers:{'cache-control':'no-store'}});return rangeResponse(row.blob,event.request,row.type||row.blob.type||'application/octet-stream')}catch(error){return new Response('IndexedDB media read failed',{status:500,headers:{'cache-control':'no-store'}})}})())});
