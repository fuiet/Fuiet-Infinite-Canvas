...
async function executeStandardVideoAsync(task,provider,model,payload){
  ...
  const xogpuStudio=String(sharedRoute.protocolFamily||sharedRoute.family||config.protocolFamily||'').toLowerCase()==='xogpu-minimax-h3'&&String(sharedRoute.requestTransport||'').toLowerCase()==='multipart';
  const body=xogpuStudio?rawBody:await portableizeLocalVideoJsonBody(rawBody,config);
  updateTask(task,{progress:8});
  ...
  while(Date.now()-started<config.timeoutMs){
    ...
    for(const template of pollTemplates){const pollPath=renderPathTemplate(template,pollCtx);try{polled=await fetchJson(joinUrl(provider.baseUrl,pollPath),{method:pollMethod,headers:providerHeaders(provider),body:['GET','HEAD'].includes(pollMethod)?undefined:(pollBody===undefined?undefined:JSON.stringify(pollBody)),timeoutMs:60000,provider});break}catch(error){lastPollError=error;if([404,405].includes(Number(error?.status))||(isXogpuVideoConfig(config)&&Number(error?.status)===501))continue;if(ProviderRuntimeCore.isRetryableProviderFailure?.(error)){retryablePollError=error;break}throw error}}
    if(!polled){
