...
test('adaptive video protocol assets keep their protocol build while runtime routing is cache-busted separately',()=>{
  for(const file of ['video-protocol-registry.js','provider-adapter-contract.js','provider-runtime-core.js']){
    assert.ok(index.includes(`${file}?v=20260901-video-wait-progress-1`),file);
  }
  assert.ok(index.includes('browser-runtime.js?v=20260902-desktop-runtime-router-1'),'browser-runtime.js');
  assert.ok(index.includes('browser-bootstrap.js?v=20260902-desktop-runtime-router-1'),'browser-bootstrap.js');
  assert.ok(index.includes('video-request-parameters.js?v=20260901-video-wait-progress-1'));
});
