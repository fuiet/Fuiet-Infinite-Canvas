...
test('Node provider gateway redacts auth headers and blocks credentialed cross-origin redirects',{concurrency:false},async()=>{
  ...
  for(const name of ['server.js','store.js','provider-adapter-contract.js','provider-runtime-core.js','local-media-result.js'])fs.copyFileSync(path.join(ROOT,name),path.join(tmp,name));
