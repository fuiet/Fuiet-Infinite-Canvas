(()=>{
  ...
  async function load(){try{const out=await api('/api/providers');const remote=out.providers||[];const local=loadLocalProviders();if(remote.length){const localById=new Map(local.map(p=>[p.id,p]));providers=remote.map(p=>({...p,...(localById.get(p.id)||{}),models:Array.isArray((localById.get(p.id)||{}).models)?(localById.get(p.id)||{}).models:p.models||[]}));saveLocalProviders(providers);}else if(local.length){providers=local;try{const restored=await restoreProvidersToServer(local);if(restored.length){const localById=new Map(local.map(p=>[p.id,p]));providers=restored.map(p=>({...p,...(localById.get(p.id)||{}),models:Array.isArray((localById.get(p.id)||{}).models)?(localById.get(p.id)||{}).models:p.models||[]}));saveLocalProviders(providers);}}catch{}}else providers=[];draftProviders=clone(providers);const q=new URLSearchParams(location.search),requested=q.get('provider');if(requested&&draftProviders.some(p=>p.id===requested))activeProvider=requested;render();$('#saveState').textContent='自动保存已开启'}catch(e){const local=loadLocalProviders();providers=local;draftProviders=clone(local);if(draftProviders.length){render();$('#saveState').textContent='已恢复本地缓存';return}$('#modelList').innerHTML=`<div class="empty-state">无法读取模型：请确认服务已启动并检查登录状态。<br><br>如果设置了访问密码，请先返回画布完成登录。</div>`}}
  async function saveAll(silent=false){
    ...
    try{
      ...
      const fresh=await api('/api/providers');providers=fresh.providers||[];draftProviders=clone(providers);saveLocalProviders(providers);
