...
function looksLikeLegacyAutoVideoRoute`);
  ...
  // Remove legacy generic video route migration semantics; keep route explicitness generic for non-video adapters.
...
test('non-Agnes video protocols are rejected instead of guessed',()=>{
  for(const id of ['kling-v3','seedance-2.0','veo-3.1','minimax-h3','vidu-q2','wan2.2','grok-video']){
    assert.throws(()=>A.resolveVideoRoute({baseUrl:'https://gateway.example.com'},{id,modality:'video'},{parameters:{}},[]),/固定为 Agnes API/);
  }
});
...
test('provider finalization removes non-Agnes video models',()=>{
  const generic=A.finalizeProvider({baseUrl:'https://gateway.example.com/v1',models:[{id:'kling-v3',modality:'video'},{id:'gpt-5',modality:'text'}]});
  assert.deepEqual(generic.models.map(x=>x.id),['gpt-5']);
  const agnes=A.finalizeProvider({baseUrl:'https://apihub.agnes-ai.com/v1',models:[{id:'kling-v3',modality:'video'}]});
  assert.equal(agnes.models.some(x=>x.id==='kling-v3'),false);
  assert.equal(agnes.models.some(x=>x.id==='agnes-video-2.5-flash'),true);
});
...
test('runtime core still parses Agnes terminal result',()=>{
  ...
  assert.equal(C.classifyAsyncPoll(raw,route,'video').state,'success');
  assert.equal(C.classifyAsyncPoll(raw,route,'video').output,'https://cdn.example.com/a.mp4');
});
`);
}
