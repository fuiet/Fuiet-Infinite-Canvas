...
test('browser provider flow uses direct request then stateless proxy fallback',()=>{assert.match(runtime,/const fetchInit=\{\.\.\.init,mode:'cors',redirect:'follow'/);assert.match(runtime,/return await rawFetch\(url,fetchInit\)/);assert.match(runtime,/return proxyFetch\(url,init\)/);assert.match(runtime,/authCandidates/);assert.match(runtime,/\/v1\/models/);assert.match(runtime,/classifyAsyncPoll/)})
