...
test('XOGPU MiniMax-H3 uses exact create poll and content endpoints',()=>{
  ...
  assert.deepEqual(route.createCandidates,['/v1/videos']);assert.deepEqual(route.pollPathCandidates,['/v1/videos/{{taskId}}','/v1/tasks/{{taskId}}']);
...
test('XOGPU multimodal references enforce HTTPS and documented media limits',()=>{
  ...
  const maxMixed=[...Array.from({length:9},(_,i)=>({type:'image',url:`https://cdn.example.com/i${i}.png`})),...Array.from({length:3},(_,i)=>({type:'video',url:`https://cdn.example.com/v${i}.mp4`})),...Array.from({length:3},(_,i)=>({type:'audio',url:`https://cdn.example.com/a${i}.wav`}))];
