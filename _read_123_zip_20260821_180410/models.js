...
const defaultModel=()=>({id:'',name:'新模型',modality:'text',enabled:true,adapterKey:'auto',videoProtocolFamily:'',videoProtocolConfig:{},operationRoutes:{},createPath:'',method:'POST',responseMode:'sync',outputPath:'',taskIdPath:'',pollPath:'',statusPath:'',progressPath:'',successValues:['completed','succeeded','success'],failureValues:['failed','error','canceled'],pollIntervalMs:1500,timeoutMs:1200000,requestTemplate:{},capabilities:{},pricing:{currency:'USD',perRequest:0,perImage:0,perSecond:0,perMillionInputTokens:0,perMillionOutputTokens:0}});
  ...
  function rowHtml(x,visibleIndex){const {p,m,i}=x;const ad=resolvedAdapter(p,m);return `<article class="model-row ${m.enabled===false?'disabled':''}" data-provider-id="${attr(p.id)}" data-model-index="${i}">
    ...
    <details class="developer-config"><summary>开发者高级设置 <span>普通用户不用填写</span></summary><div class="details-grid developer-grid">
      ${field('视频协议族',`<select data-field="videoProtocolFamily"><option value="" ${!m.videoProtocolFamily?'selected':''}>自动识别</option>${['kling','seedance','minimax-hailuo','vidu','veo','sora-openai','wan','grok','generic-video'].map(k=>`<option value="${k}" ${m.videoProtocolFamily===k?'selected':''}>${k}</option>`).join('')}</select><small>仅视频模型需要。普通用户保持“自动识别”。</small>`)}
      ...
      ${field('模型视频协议 Override JSON',`<textarea data-field="videoProtocolConfig">${esc(JSON.stringify(m.videoProtocolConfig||{},null,2))}</textarea><small>可覆盖 createPath/createCandidates、pollPath/pollPathCandidates、taskIdPaths、statusPaths、outputPaths、contentPathCandidates。仅特殊供应商需要。</small>`,'full')}
  ...
  function readRow(row){const pid=row.dataset.providerId, idx=Number(row.dataset.modelIndex),p=draftProviders.find(x=>x.id===pid),m=p?.models?.[idx];if(!m)return;const val=f=>$(`[data-field="${f}"]`,row)?.value??'';m.name=val('name');m.id=val('id').trim();m.modality=val('modality')||m.modality;m.modalitySource='user';m.adapterKey=val('adapterKey')||m.adapterKey||'auto';m.videoProtocolFamily=val('videoProtocolFamily').trim();m.responseMode=val('responseMode')||m.responseMode||'sync';m.enabled=$('[data-field="enabled"]',row)?.checked!==false;m.createPath=val('createPath').trim();m.outputPath=val('outputPath').trim();m.taskIdPath=val('taskIdPath').trim();m.pollPath=val('pollPath').trim();m.statusPath=val('statusPath').trim();m.progressPath=val('progressPath').trim();m.successValues=val('successValues').split(',').map(x=>x.trim()).filter(Boolean);m.failureValues=val('failureValues').split(',').map(x=>x.trim()).filter(Boolean);m.pollIntervalMs=Number(val('pollIntervalMs')||1500);m.timeoutMs=Number(val('timeoutMs')||1200000);try{m.capabilities=JSON.parse(val('capabilities')||'{}');delete m.__capInvalid}catch{m.__capInvalid=true}try{m.pricing=JSON.parse(val('pricing')||'{}');delete m.__priceInvalid}catch{m.__priceInvalid=true}try{m.videoProtocolConfig=JSON.parse(val('videoProtocolConfig')||'{}');delete m.__videoProtocolInvalid}catch{m.__videoProtocolInvalid=true}try{m.requestTemplate=JSON.parse(val('requestTemplate')||'{}');delete m.__reqInvalid}catch{m.__reqInvalid=true}try{m.operationRoutes=JSON.parse(val('operationRoutes')||'{}');delete m.__opInvalid}catch{m.__opInvalid=true}}
  ...
  async function saveAll(silent=false){
    ...
    clearTimeout(autoSaveTimer);
    syncVisibleRows();
    const invalid=flattened().find(({m})=>m.__capInvalid||m.__priceInvalid||m.__videoProtocolInvalid||m.__reqInvalid||m.__opInvalid||!String(m.id||'').trim()||!m.modality);
